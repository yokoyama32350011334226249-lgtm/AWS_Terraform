...
DeepARアルゴリズムを用いて、過去の株価データから将来の株価を予測します。
S3の作成、Sagemakerノートブックインスタンスの作成、Sagemakerモデルのエンドポイントの作成は自動化されています。
モデルの作成、推論実行はSagemakerノートブックインスタンスを使って手動で行います。

## 使用手順

①terraform apply
②AWSコンソールのSageMakerのノートブックインスタンスを表示してJupyterLabを開く
③JupyterLabにdataフォルダのCSVファイルを格納
④JupyterLabにcodeフォルダのファイルを格納して以下の順に実行
  1.nikkei_prediction.ipynb (モデル作成)
  2.nikkei_prediction_inference.ipynb (推論実行)
  3.predict_result.ipynb (結果表示)
⑤S3のモデルのパスを確認
⑥sagemaker_endpoint.tf のコメントアウトを解除
  ⑤で確認したモデルのパスをコードに反映
⑦terraform apply
...
- **aws_s3_bucket: `example`**
  - SageMaker の学習データ・モデル成果物を保存するバケット
  - バケット名は変数 `s3_bucket` で指定（`terraform.tfvars` で設定）
  - バージョニング有効
  - パブリックアクセス完全ブロック

### 1. **IAM ロール・ポリシー** (`sagemaker.tf`)
...
### 2. **Notebook インスタンス** (`sagemaker.tf`)
- **aws_sagemaker_notebook_instance: `deepar_notebook`**
  - JupyterLab 環境を提供
  - インスタンスタイプ: ml.t3.medium
  - **用途**: 学習・推論を手動で実行

### 3. **モデル** (`sagemaker_endpoint.tf` ※初期はコメントアウト)
- **aws_sagemaker_model: `deepar_model`**
  - 学習済みモデルをモデルレジストリに登録
  - **注**: Notebook で学習完了後、`model_data_url` を実際の S3 パスに更新しコメントアウトを解除
  - ECR の DeepAR イメージを `data.aws_sagemaker_prebuilt_ecr_image` で動的取得

### 4. **エンドポイント設定** (`sagemaker_endpoint.tf` ※初期はコメントアウト)
- **aws_sagemaker_endpoint_configuration: `deepar_endpoint_config`**
  - モデルをデプロイするための設定
  - インスタンスタイプ: ml.m5.large
  - インスタンス数: 1

### 5. **推論エンドポイント** (`sagemaker_endpoint.tf` ※初期はコメントアウト)
...
| 出力値 | 説明 | 定義ファイル |
|--------|------|-------------|
| `sagemaker_notebook_instance_name` | Notebook インスタンス名 | `sagemaker_endpoint.tf`（コメントアウト） |
| `notebook_instance_url` | Notebook インスタンスへのアクセス URL | `sagemaker_endpoint.tf`（コメントアウト） |
| `sagemaker_model_name` | モデル登録名 | `sagemaker_endpoint.tf`（コメントアウト） |
| `sagemaker_endpoint_name` | 推論エンドポイント名 | `sagemaker_endpoint.tf`（コメントアウト） |
| `sagemaker_role_arn` | SageMaker 実行ロール ARN | `sagemaker_endpoint.tf`（コメントアウト） |
| `s3_training_data_location` | トレーニングデータのS3パス | `sagemaker_endpoint.tf`（コメントアウト） |
| `s3_output_path` | モデル出力のS3パス | `sagemaker_endpoint.tf`（コメントアウト） |

> **注意**: 出力値はすべて `sagemaker_endpoint.tf` のコメントアウトを解除した後に利用可能になります。
...
### S3 バケット・パスの確認
- バケット名は `variables.tf` の `s3_bucket` 変数で管理（デフォルト: `"sagemaker-test"`）
- `terraform.tfvars` に実際のバケット名を設定してください
- `training_data_uri = "s3://<s3_bucket>/nikkei-deepar/train"`
- `output_path = "s3://<s3_bucket>/nikkei-deepar/output"`

環境に合わせて、**`terraform.tfvars` の `s3_bucket` を変更してください**。
...
│   ├── variables.tf             # 変数定義（s3_bucket など）
│   ├── terraform.tfvars         # 変数の実値設定（バケット名など）
│   ├── s3.tf                    # S3 バケット定義
│   ├── sagemaker.tf             # IAM ロール・Notebook インスタンス定義
│   ├── sagemaker_endpoint.tf    # モデル・エンドポイント定義（初期はコメントアウト）
